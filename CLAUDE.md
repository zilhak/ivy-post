...
| 용어 | 설명 |
|------|------|
| **툴바 팝업** | 크롬 우측 상단 익스텐션 아이콘 클릭 시 나오는 드롭다운 (`popup.html`) |
| **액션 팝업** | DOM 요소 선택 시 나오는 1차 메뉴. 수행할 작업을 선택 |
| **댓글 팝업** | 액션 팝업에서 "댓글 작성" 선택 시 나오는 2차 폼 |
| **댓글 상세** | 핀 클릭 시 기존 댓글 내용을 보여주는 팝업 |

### 인터랙션 흐름

```
선택 모드 → DOM 요소 클릭 → 액션 팝업 (메뉴) → 항목 선택 → 2차 팝업 (댓글 팝업, 전송 팝업 등)
```
